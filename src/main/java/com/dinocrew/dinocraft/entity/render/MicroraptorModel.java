...
public class MicroraptorModel extends HierarchicalModel<Microraptor> {
    ...
    public MicroraptorModel(ModelPart root) {
    ...
    @Override
    public void setupAnim(Microraptor entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (!entity.isOnGround()) {
            ...
            this.right_wing1.zRot = Mth.cos(ageInTicks / 5) / 3;
            this.left_wing1.zRot = -Mth.cos(ageInTicks / 5) / 3;

            this.right_wing2.yRot = 0;
            this.left_wing2.yRot = 0;
            this.right_wing2.xRot = 0;
            this.left_wing2.xRot = 0;
            this.right_wing2.zRot = Mth.cos(ageInTicks / 5) / 3;
            this.left_wing2.zRot = -Mth.cos(ageInTicks / 5) / 3;

            this.right_wing3.yRot = 0;
            this.left_wing3.yRot = 0;
            this.right_wing3.xRot = 0;
            this.left_wing3.xRot = 0;
            this.right_wing3.zRot = Mth.cos(ageInTicks / 5) / 3;
            this.left_wing3.zRot = -Mth.cos(ageInTicks / 5) / 3;
            ...
            this.tail1.yRot = -Mth.cos(ageInTicks / 5) / 5;
            this.tail2.yRot = -Mth.cos(ageInTicks / 5) / 5;
            this.tail3.yRot = -Mth.cos(ageInTicks / 5) / 5;

            this.tail1.zRot = Mth.cos(ageInTicks / 5) / 5;
            this.tail2.zRot = Mth.cos(ageInTicks / 5) / 5;
            this.tail3.zRot = Mth.cos(ageInTicks / 5) / 5;

            this.head.xRot = 0.7854F;
            this.neck.xRot = -0.6981F;

            this.tail1.xRot = -Mth.sin(ageInTicks / 5) / 20 - 20 * pi / 180;
            this.tail2.xRot = Mth.sin(ageInTicks / 5) / 20 + 10 * pi / 180;
            this.tail3.xRot = -Mth.sin(ageInTicks / 5) / 20 - 10 * pi / 180;

            this.left_leg.xRot = Mth.cos(limbSwing * 0.6662F + 3.1415927F) * 1.4F * limbSwingAmount / 2 - 0.3491F;
            this.right_leg.xRot = -Mth.cos(limbSwing * 0.6662F + 3.1415927F) * 1.4F * limbSwingAmount / 2 - 0.3491F;

            this.left_knee.xRot = Mth.cos(limbSwing * 0.6662F + 3.1415927F) * 1.4F * limbSwingAmount / 2 + 0.6981F;
            this.right_knee.xRot = -Mth.cos(limbSwing * 0.6662F + 3.1415927F) * 1.4F * limbSwingAmount / 2 + 0.6981F;
